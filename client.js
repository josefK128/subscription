...
web3.eth.getAccounts((err, accounts) => {
  ...
  .then((subscriptionInstance) => {
    ...
    const oraclePromises = [
      subscriptionInstance.getPrice(),  // Get currently stored BTC Cap
      subscriptionInstance.update({from: accounts[0]})  // Request oracle to update the information
    ]
    ...
    .then((result) => {
        if(result[0] > 0){
          ...
          subscriptionInstance.subscribe('jack@gmail.com', {from: accounts[1]});
        }else{
